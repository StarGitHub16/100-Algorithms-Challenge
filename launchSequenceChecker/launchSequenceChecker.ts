export function launchSequenceChecker(systemNames: string[], stepNumbers: number[]): boolean {
    ...
    const launchCodes = {}; //Create an object to store if a launch code has been called.


    for( let i = 0; i < systemNames.length; i++) {
        if(launchCodes.hasOwnProperty(systemNames[i])) { //Check to see if it has been called.
