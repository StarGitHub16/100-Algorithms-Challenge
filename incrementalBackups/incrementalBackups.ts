//A bit of a medium challenge. Algorithm from doto company at codefights.
...
export function incrementalBackups(lastBackupTime: number, changes: number[][]): number[] {
        ...
        for(let change of changes) {
            const fileBackuptime = change[0];  //Only doing consts to make the code more readable.
            const fileId = change[1];

            if (fileBackuptime > lastBackupTime) {

                if(!fileIds.includes(fileId)) { //Only do it if the file changes any at all.
